	...
	return &discordgo.InteractionResponse{
		...
		Data: &discordgo.InteractionResponseData{
			Content: Lock + " Security Overview " + Lock + "\n\n" +
				...
				"`/" + LOGIN + "` vs. `/" + LOGINCOOKIE + "`\n\n" +
				"If you specify your login details with `/" + LOGIN + "`, SlickShift uses your username/password to login to SHiFT on your behalf, obtain session cookies, and then encrypt and store those cookies for later. " +
				"Once this process completes, *your username/password are completely forgotten/discarded by the bot.*\n\n" +
				"`/" + LOGINCOOKIE + "` takes a different approach, and instead requires users to provide session cookies directly; no username or password. This is more secure, but is " +
